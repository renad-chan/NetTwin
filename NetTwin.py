...
#import console_network_analysis as cna
...
class FullScreenLoginSystem:
    ...
    def capture_trafic(self):
        if(self.devices):
            ...
            if digital_twins:
                ...
                #print("\n--- Network Summary ---")
                #print(f"Total Packets: {sum(data['traffic_count'] for data in digital_twins.values())}")
                #print(f"Total Traffic: {sum(data['total_bytes'] for data in digital_twins.values())} bytes")
                #print(f"Total ICMP Requests: {sum(data['icmp_requests'] for data in digital_twins.values())}")
                #print(f"Total ICMP Replies: {sum(data['icmp_replies'] for data in digital_twins.values())}")
                #print(f"Total Delay: {total_delay:.2f} seconds")
            else:
    ...
    def draw_digital_twin(self,devices,digital_twins, router_ip, rootFrame):
        ...
        #plt.show()  # Show the graph instead of saving it
